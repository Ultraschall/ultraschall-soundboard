...
void SoundboardAudioProcessor::setCurrentProgram(int index) {
  ...
  if (index == ProgramNumberInit) {
    ...
    samplePlayers.clear();
    SoundboardAudioProcessorEditor *editor =
        (SoundboardAudioProcessorEditor *)getActiveEditor();
    if (editor != nullptr) {
      editor->refresh();
    }
    updateHostDisplay();
...
void SoundboardAudioProcessor::setStateInformation(const void *data,
  ...
  if (program.isValid()) {
    String directoryString =
        program.getProperty(DirectoryIdentifier, String::empty).toString();
...
void SoundboardAudioProcessor::openDirectory(File directory) {
  ...
  while (iterator.next()) {
    if (formatManager.findFormatForFileExtension(
            iterator.getFile().getFileExtension()) != nullptr &&
  ...
  SoundboardAudioProcessorEditor *editor =
      static_cast<SoundboardAudioProcessorEditor *>(getActiveEditor());
  if (editor) {
    editor->refresh();
  }
...
void SoundboardAudioProcessor::oscSendPlayerUpdate() {
  for (int index = 0; index < samplePlayers.size(); index++) {
    Player *samplePlayer = SamplePlayerAtIndex(index);
    String address =
        "/ultraschall/soundboard/player/" + String(index + 1) + "/";
