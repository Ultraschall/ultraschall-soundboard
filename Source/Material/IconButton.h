...
namespace Material {
    ...
    public:
        ...
        IconButton(Material::Icon &icon) : Button("IconButton"), icon(icon) {
    ...
    public:
        ...
        IconToggleButton(Material::Icon &iconInactive, Material::Icon &iconActive) : Button("IconToggleButton"), iconActive(iconActive), iconInactive(iconInactive) {
        ...
        void paintButton(Graphics &g, bool isMouseOverButton, bool isButtonDown) override {
            if (getToggleState()) {
                if (drawableActive != nullptr) {
                    drawableActive->drawWithin(g, getLocalBounds().toFloat().withSizeKeepingCentre(iconSize, iconSize), RectanglePlacement(RectanglePlacement::Flags::centred), 1.0f);
                }
            } else {
                if (drawableInactive != nullptr) {
                    drawableInactive->drawWithin(g, getLocalBounds().toFloat().withSizeKeepingCentre(iconSize, iconSize), RectanglePlacement(RectanglePlacement::Flags::centred), 1.0f);
                }
            }
        }
