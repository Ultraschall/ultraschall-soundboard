...
SoundboardAudioProcessorEditor::SoundboardAudioProcessorEditor(
    ...
    : AudioProcessorEditor(&p), processor(p), mPauseState(true),
      mTimerState(false) {
  ...
  fadeOutLabel->setColour(Label::ColourIds::textColourId,
                          Colours::grey.brighter());
  fadeOutLabel->setText("Ausblendzeit: " +
                            String(processor.getFadeOutSeconds()) + "s",
  ...
  addAndMakeVisible(settingsButton = new ImageButton());
...
void SoundboardAudioProcessorEditor::paint(Graphics &g) {
  ...
  // g.fillAll(Colour::fromRGB(255, 0, 255));
  ...
  g.drawText(CharPointer_UTF8("OSC aktivit\xc3\xa4t"), 30, getHeight() - 27,
             100, 24, Justification::left);
  ...
  if (oscIndicator) {
    ...
    g.drawText(FA_DOT_CIRCLE_O, 3, getHeight() - 27, 24, 24,
               Justification::centred, true);
    g.setColour(Colours::grey.brighter());
  }
  g.drawText(FA_CIRCLE_O, 3, getHeight() - 27, 24, 24, Justification::centred,
             true);
  g.drawText(FA_COG, getWidth() - 50, getHeight() - 27, 24, 24,
             Justification::centred, true);
}
...
void SoundboardAudioProcessorEditor::resized() {
  ...
  tableListBox->setBounds(3, 32, getWidth() - 6, getHeight() - 58);
  tableListBox->getHeader().setColumnWidth(RowIdFile, getWidth() - 268);
  resizer->setBounds(getWidth() - 16, getHeight() - 16, 16, 16);

  settingsButton->setBounds(getWidth() - 50, getHeight() - 27, 24, 24);
}
...
void SoundboardAudioProcessorEditor::paintCell(Graphics &g, int rowNumber,
  ...
  case RowIdTime: {
    ...
    if (!processor.SamplePlayerAtIndex(rowNumber)->isPlayed()) {
      g.fillRect(0, 0,
                 (int)(width *
                       processor.SamplePlayerAtIndex(rowNumber)->getProgress()),
                 height);
    } else {
...
void SoundboardAudioProcessorEditor::cellClicked(int rowNumber, int columnId,
  ...
  if (columnId == RowIdLoop) {
    ...
    if (processor.SamplePlayerAtIndex(rowNumber)->getState() ==
        Player::Playing) {
...
void SoundboardAudioProcessorEditor::timerCallback(int timerID) {
  if (timerID == 1) {
    fadeOutSlider->setValue(processor.getFadeOutSeconds(),
                            NotificationType::dontSendNotification);
    if (oscIndicator != processor.receivedOscMessages()) {
      oscIndicator = processor.receivedOscMessages();
      repaint();
    }
  } else if (timerID == 0) {
