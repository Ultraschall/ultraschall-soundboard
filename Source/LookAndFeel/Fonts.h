...
namespace Material
{
	...
	public:
		...
		const Font Body1() { return Font(Typefaces::getInstance()->Body1).withHeight(px(16.0f)); }
		const Font Body2() { return Font(Typefaces::getInstance()->Body2).withHeight(px(14.0f)); }
		const Font Subtitle1() { return Font(Typefaces::getInstance()->Subtitle1).withHeight(px(16.0f)); }
		const Font Subtitle2() { return Font(Typefaces::getInstance()->Subtitle2).withHeight(px(14.0f)); }
		const Font Button() { return Font(Typefaces::getInstance()->Button).withHeight(px(14.0f)); }
		const Font Caption() { return Font(Typefaces::getInstance()->Caption).withHeight(px(12.0f)); }
		const Font Overline() { return Font(Typefaces::getInstance()->Overline).withHeight(px(12.0f)); }
	};
} // namespace Material
