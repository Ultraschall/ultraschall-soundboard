...
namespace Material
{
	...
	public:
		...
		void paintButton(Graphics &g, bool /*isMouseOverButton*/, bool /*isButtonDown*/) override
		{
			...
			p.addEllipse(0.0f, 0.0f, static_cast<float>(getWidth()), static_cast<float>(getHeight()));
			//shadow.drawForPath(g, p);
			g.setColour(findColour(ColourIds::containerColourId));
			g.fillPath(p);

			drawable->drawWithin(g, getLocalBounds().toFloat().withSizeKeepingCentre(px<float>(iconSize), px<float>(iconSize)), RectanglePlacement(RectanglePlacement::Flags::centred), 1.0f);
		}
